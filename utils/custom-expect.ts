...
declare global {
    namespace PlaywrightTest {
        interface Matchers<R, T> {
            ...
            shouldMatchSchema(dirName: string, fileName: string, createSchemeFlag?: boolean): Promise<R>
        }
    }
}

export const expect = baseExpect.extend({
    async shouldMatchSchema(received: any, dirName: string, fileName: string, createSchemeFlag?: boolean) {
        let pass: boolean;
        let message: string = ''
        try {
            await validateSchema(dirName, fileName, received, createSchemeFlag)
            pass = true
